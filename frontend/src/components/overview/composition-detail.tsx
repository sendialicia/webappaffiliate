...
export function CompositionDetail({
  ...
  return (
    <div
      // Eases open rather than snapping in, so the click visibly leads to the panel.
      className="mt-3.5 animate-in fade-in-0 slide-in-from-top-2 rounded-xl border border-[var(--ov-line)] bg-[var(--ov-fill1)] p-4 duration-300"
    ...
    >
      ...
      {/* Who moved this slice: the creators behind the gain and behind the loss. */}
      <div className="mt-4">
        <CreatorDrivers query={query} slices={[{ field: result.dimension, value: row.name }]} />
      </div>
    </div>
  )
}
