...
import type { PidLevel, TtQuadrantPreset } from "@/types/tiktok-pid"
...
export interface TtFilterDraft {
  brand: string[]
  detail: DetailFilters
}

interface TiktokPidState {
  brand: string[]
  detail: DetailFilters
  draft: TtFilterDraft
  preset: DatePreset
  ...
  setQuadrant: (quadrant: TtQuadrantPreset) => void
...
export const useTiktokPidFilters = create<TiktokPidState>((set) => ({
  brand: [],
  detail: {},
  draft: { brand: [], detail: {} },
  preset: "mtd",
  ...
  hydrateFromParams: (params) =>
    set((state) => {
      ...
      return {
        ...
        detail,
        draft: { brand, detail },
        countFilter: (params.get("count") as TiktokPidState["countFilter"]) ?? state.countFilter,
        ...
        quadrant: (params.get("quad") as TtQuadrantPreset) ?? state.quadrant,
...
export function ttFiltersToParams(state: TiktokPidState): URLSearchParams {
